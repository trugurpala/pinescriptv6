...
| Codex Desktop skill | `.agents/skills/pine-script-agent-kit/SKILL.md`; `.agents/skills/pine-script-agent-kit/agents/openai.yaml` | In Codex, run `$skill-installer https://github.com/trugurpala/pinescriptv6/tree/main/.agents/skills/pine-script-agent-kit`. If the skill does not appear after installation, restart Codex. | Invoke `$pine-script-agent-kit`, confirm that Codex names the loaded skill, then run the smoke prompt below. | “Use Pine Script Agent Kit to review this Pine Script v6 code; follow the smoke-prompt evidence format.” |
...
| Devin | `AGENTS.md` | Keep it at the project root. | Confirm the repo knowledge lists `AGENTS.md` under Accessed Knowledge, then run the smoke prompt. | “Use the Devin repository knowledge to review this Pine v6 code; follow the smoke-prompt evidence format.” |
| Windsurf | `AGENTS.md` primary; `.windsurf/rules/pine-script-agent-kit.md` fallback workspace-rule bridge; `.windsurfrules` legacy compatibility | Keep `AGENTS.md` at the project root and retain both generated bridges. | Open a Pine file in Windsurf and run the smoke prompt. | “Use the active Windsurf Pine rule to review this Pine v6 code; follow the smoke-prompt evidence format.” |
...
| Zed | `.cursorrules` | Keep `.cursorrules` at the project root. Zed uses the first matching supported instruction file; because `.cursorrules` precedes `AGENTS.md`, it is the selected supported surface in this full repository. | Open the full repository in Zed and run the smoke prompt from a Pine file. | “Use the Zed-selected `.cursorrules` guidance to review this Pine v6 code; follow the smoke-prompt evidence format.” |
